...
# The agent starts at 0"""
    ...
    print()
    print('# This specific file was generated with parameters:')
    print(f'# {config}')
    print()
    ...
    #     or s.cell.idx == len(cell_space) - config.n - 1
    # ]
    ...
    for a in action_space.elems():
        ...
        if a.value == 'N':
            for s in state_space.elems():
                ...
                elif s.cell.value == len(cell_space) - config.n - 1:
                    s1 = copy(s)
                    s1.cell.value = 0
                    print(f'T: {afmt(a)}: {sfmt(s)}: {sfmt(s)} 0.0')
                    print(f'T: {afmt(a)}: {sfmt(s)}: {sfmt(s1)} 1.0')
            ...
            for s in state_space.elems():
                if 0 < s.cell.value <= config.n:
                    ...
                    s1 = copy(s)
                    s1.cell.value = len(cell_space) - config.n - 1
                    print(f'T: {afmt(a)}: {sfmt(s)}: {sfmt(s)} 0.0')
                    print(f'T: {afmt(a)}: {sfmt(s)}: {sfmt(s1)} 1.0')
            ...
            for s in state_space.elems():
                ...
                if (
                    ...
                    or 3 * config.n < s.cell.value < len(cell_space) - 1
                ):
                    s1 = copy(s)
                    s1.cell.value += 1
                    print(f'T: {afmt(a)}: {sfmt(s)}: {sfmt(s)} 0.0')
            ...
            for s in state_space.elems():
                if config.n <= s.cell.value < 2 * config.n:
                    ...
                    2 * config.n < s.cell.value <= 3 * config.n
                    or 3 * config.n + 1 < s.cell.value < len(cell_space)
                ):
                    s1 = copy(s)
                    s1.cell.value -= 1
                    print(f'T: {afmt(a)}: {sfmt(s)}: {sfmt(s)} 0.0')
    ...
    for s1 in state_space.elems():
        if s1.cell.value < len(cell_space) - 1:
            print(f'O: *: {sfmt(s1)}: o{s1.cell.value} 1.0')
        else:  # if s1.cell.value == len(cell_space) - 1:
